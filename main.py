...
class NlpClassifier(object):
    def __init__(self,arr):
        # initialize the variables used in the question
        self.mapp=dict()
        self.mapp2=dict()
        self.N=len(arr)
        self.m_val=1
        ...
        self.totalLabels=[0 for i in range(5)]
    ...
    def calculate_probability(self):
        ...
        for i in range(5):
            self.totalLabels[i]=self.totalLabels[i]/self.labelCount
    ...
    def predict_from_file(self,filename):
        # returns an array of tuples = (question,prediction)
        file = open(filename,"r")
        res=[]
        for line in file:
            if line == '\n':
